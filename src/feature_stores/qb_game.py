...
from src.extracts import get_play_by_play, collect_weekly_espn_player_stats, get_schedules, load_players
...
class QBDataLoader():
    ...
    def __init__(self, load_seasons):
        ...
        ## load external data ##
        self.db = {}
        self.db['player_stats'] = pd.concat([collect_weekly_espn_player_stats(season, season_type=None) for season in load_seasons])
        self.db['games'] = get_schedules(load_seasons)
    ...
    def retrieve_player_meta(self, df):
        ...
        def add_missing_draft_data(df):
            ...
            missing_draft = pd.read_csv("https://github.com/greerreNFL/nfeloqb/raw/refs/heads/main/nfeloqb/Manual%20Data/missing_draft_data.csv",index_col=0)
    ...
    def calculate_passer_rating(self, df):
        # Avoid division by zero by replacing zero attempts with a small number or assigning 0 rating
        df['attempts'] = df['attempts'].replace(0, float('nan'))  # Replace 0 attempts with NaN to avoid invalid calculations

        # Calculate components with NaN-safe operations
        a = ((df['completions'] / df['attempts'] - 0.3) * 5).clip(0, 2.375).fillna(0)
        b = ((df['passing_yards'] / df['attempts'] - 3) * 0.25).clip(0, 2.375).fillna(0)
        c = (df['passing_tds'] / df['attempts'] * 20).clip(0, 2.375).fillna(0)
        d = (2.375 - (df['interceptions'] / df['attempts'] * 25)).clip(0, 2.375).fillna(0)
    ...
    def pull_data(self):
        ...
        while df is not None:
            ...
            df['passer_rating'] = self.calculate_passer_rating(df)
...
def load_data(load_seasons):
    ...
    qbdl_obj = QBDataLoader(load_seasons)
    player_df = qbdl_obj.model_df
    del qbdl_obj

    return data, player_df


def make_qb_game_feature_store(load_seasons):
    data = load_data(load_seasons)